...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
	...
	/**
	 * FIX 3 — Inject EmailService so we can send a login notification email
	 * after every successful OAuth2 authentication (Google / GitHub).
	 *
	 * Previously, sendLoginNotificationEmail() was only called from
	 * AuthenticationService.login() (the email+password flow). OAuth2 logins
	 * went through CustomOAuth2UserService and never triggered any email, so
	 * users never received a "new login" notification when signing in with Google.
	 *
	 * EmailService methods are @Async — the email is sent on a background thread
	 * and will NOT block or throw inside the OAuth2 redirect flow even if the
	 * SMTP server is slow or temporarily unavailable.
	 */
	...
	private UserPrincipal processOAuth2User(OAuth2UserRequest userRequest, OAuth2User oAuth2User) {
		...
		UserPrincipal principal = userRepository.findByEmail(info.email)
				.map(existing -> updateExistingUser(existing, info, attributes))
				.orElseGet(() -> registerNewUser(info, attributes));

		// FIX — Pass the user's name so the greeting is personalized ("Hi John," instead of "Hi,").
		// sendLoginNotificationEmail is @Async — runs on a background thread, never blocks the
		// OAuth2 redirect flow, and swallows any SMTP errors internally.
		emailService.sendLoginNotificationEmail(info.email, info.name);
	...
	private UserPrincipal registerNewUser(OAuthUserInfo info, Map<String, Object> attributes) {
		...
		// Send a welcome email for first-time OAuth2 registrations, just like the
		// email+password registration flow does in AuthenticationService.register().
		// This is @Async so it never blocks the OAuth2 redirect.
		emailService.sendRegistrationEmail(saved.getEmail(), saved.getName() != null ? saved.getName() : "User");
